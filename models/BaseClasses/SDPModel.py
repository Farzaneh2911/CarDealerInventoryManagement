...
class SDPModel(ABC):
    ...
    def build_decision(self, decision_dict):
        """
        ...
        return Decision(
        restock=decision_dict["restock"],
        price=decision_dict["price"],
        discount=decision_dict["discount"],
    )
    ...
    # Execute transition function and add new state to exog_info dict
        new_state = self.transition_fn(decision, exog_info)
    ...
    # Build new state from updated exog_info
       

    # Update time counter
        self.update_t()

    # Return the new state and a flag indicating if the episode is finished
